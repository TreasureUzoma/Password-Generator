...
function MainContent() {
    ...
    useEffect(() => {
        handleGeneratePassword(); // Regenerate password whenever checkedStates change
    }, [checkedStates, passwordLength]); // Dependency array to watch changes
    ...
    const shufflePassword = finalPassword => {
        ...
        const interval = setInterval(() => {
            setDisplayedPassword(
                Array.from({ length: passwordLength }, () =>
                    getRandomChar()
                ).join("")
            );
            currentShuffle++;
                                ...
                                <label
                                    ...
                                    style={{
                                        color:
                                            key === "lowerCase"
                                                ? "#6b7d7f"
                                                : "inherit" // Slightly darker color for disabled
                                    }}
                                >
                                    {key === "easyToRemember"
                                        ? "Easy to Remember"
